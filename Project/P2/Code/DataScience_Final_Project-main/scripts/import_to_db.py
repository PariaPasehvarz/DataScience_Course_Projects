import os
import pandas as pd
from sqlalchemy import Column, Integer, String, Float, Text, ForeignKey, MetaData, Table, text
...
def import_to_database(engine: Engine, df=None, csv_path=None, table_name='movies', clear_existing=True):
    ...
    with engine.begin() as conn:
        ...
        for orig_index, genres_list in movie_to_genres.items():
            movie_id = movie_id_map.get(orig_index)
            for genre in genres_list:
                genre_id = genre_name_to_id.get(genre)
                if genre_id and movie_id:
                    conn.execute(movie_genres.insert().values(movie_id=movie_id, genre_id=genre_id))
